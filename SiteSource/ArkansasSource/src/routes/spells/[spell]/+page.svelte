<script lang="ts">
    ...
    onMount(async() => {
        info = await GetSpell("https://raw.githubusercontent.com/luax7/ArkansasTTRPG/master/Sistema/Magias/"+data.Spell+".md");
...
<div class="Selector">
        ...
        <button onclick={ToggleSpellInList}><p>{IsInList ? "remove from list" : "add to list"}</p></button>
</div>
<div class="MainContainer">
    ...
    <div class="Header">
        ...
        <div class="BasicInfo">
            <p class="headertext basicinfo"><span>Feitiço de {info?.Props.Tipo} de nivel {info?.Props.nivel}</span></p>
            <p class="headertext"><span>Custo</span> {info?.Props.custo}</p>
            <p class="headertext"><span>Duraçao</span> {info?.Props.duração}</p>
            <p class="headertext"><span>Alcance</span>  {info?.Props.alcance}</p>
            <p class="headertext"><span>Alvo</span> {info?.Props.alvo}</p>
            <p class="headertext"><span>Dificuldade</span> {info?.Props.dificuldade}</p>
            <p class="headertext"><span>Cracteristicas</span> {info?.Props.caracteristicas}</p>
            <p class="headertext"><span>Açao</span> {info?.Props.açao}</p>
            <p class="headertext"><span>Componentes</span> {info?.Props.componentes}</p>
            <p class="headertext"><span>Materiais</span> {info?.Props.materiais}</p>
        </div>
    </div>
...
<style>
    * {
    ...
    .Selector::after{
        ...
        width: 100vw;
        height: 2vh;
        background: linear-gradient(to bottom, rgba(233,233,233,1) 0%,rgba(233,233,233,0) 100%); /* W3C, IE10+, FF16+, Chrome26+, Opera12+, Safari7+ */
        position: absolute;
        top: 10vh;
        left: 0px;
    }
    ...
    .Header::after{
        ...
        width: 90vw;
        height: 1px;
        background: linear-gradient(to right, rgba(255,255,255,0) 0%,rgba(25,25,25,1) 18%,rgba(0,0,0,1) 20%,rgba(0,0,0,1) 51%,rgba(0,0,0,1) 80%,rgba(255,255,255,0) 100%); /* W3C, IE10+, FF16+, Chrome26+, Opera12+, Safari7+ */
    }
    .MainContainer{
        width: 100vw;
        height: 100vh;
